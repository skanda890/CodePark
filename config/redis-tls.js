...
const createRedisClient = async () => {
  try {
    ...
    if (redisTLS) {
      ...
      const clientCertPath = process.env.REDIS_CLIENT_CERT || '/etc/redis/client.crt';
      const clientKeyPath = process.env.REDIS_CLIENT_KEY || '/etc/redis/client.key';

      // Check if certificate files exist
      if (!fs.existsSync(caCertPath) || !fs.existsSync(clientCertPath) || !fs.existsSync(clientKeyPath)) {
        logger.warn('Redis TLS certificate files not found, falling back to non-TLS connection');
      } else {
        options.tls = {
