...
const connectDatabase = async () => {
  try {
    ...
    if (mongoTLS) {
      ...
      const clientCertPath = process.env.MONGODB_CLIENT_CERT || '/etc/mongodb/client.crt';
      const clientKeyPath = process.env.MONGODB_CLIENT_KEY || '/etc/mongodb/client.key';

      // Check if certificate files exist
      if (!fs.existsSync(caCertPath) || !fs.existsSync(clientCertPath) || !fs.existsSync(clientKeyPath)) {
        logger.warn('MongoDB TLS certificate files not found, falling back to standard connection');
      } else {
        options.ssl = true;
        ...
        logger.info('MongoDB TLS enabled with certificate-based authentication');
      }
    }
