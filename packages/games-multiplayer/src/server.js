...
io.on('connection', (socket) => {
  console.log(`[${new Date().toISOString()}] User connected: ${socket.userId} (${socket.username})`);
  ...
  socket.on('join_room', (roomId) => {
    ...
    console.log(`[${new Date().toISOString()}] Player ${socket.username} joined room ${roomId} (${room.players.length}/${room.maxPlayers})`);
  ...
  socket.on('player_move', (moveData) => {
    ...
    if (validateMove(room.gameState, moveData, socket.userId)) {
      ...
      console.log(`[${new Date().toISOString()}] Move by ${socket.username}: ${JSON.stringify(moveData)}`);
    } else {
  ...
  socket.on('disconnect', () => {
    ...
    if (room) {
      ...
      if (room.players.length === 0) {
        ...
        console.log(`[${new Date().toISOString()}] Room ${player.roomId} deleted (empty)`);
      }
    }
...
server.listen(PORT, () => {
  ...
  console.log(`║  Running on port ${PORT}${' '.repeat(19 - PORT.toString().length)}║`);
  console.log(`║  Environment: ${process.env.NODE_ENV || 'development'}${' '.repeat(22 - (process.env.NODE_ENV || 'development').length)}║`);
  console.log(`╚════════════════════════════════════════╝\n`);
});
