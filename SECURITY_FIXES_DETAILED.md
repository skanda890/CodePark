...
if (nodeEnv === 'production' && !origin) {
  return res.status(403).json({  // ✗ Blocks ALL non-origin requests
...
function decideCors({ origin, env, allowedOrigins, path }) {
  ...
  if (origin) {
    return { log: { level: 'warn', msg: 'Unauthorized origin', ctx: { origin } } }
  }
...
return (req, res, next) => {
  ...
  if (decision.log) logger[decision.log.level](decision.log.ctx, decision.log.msg)
