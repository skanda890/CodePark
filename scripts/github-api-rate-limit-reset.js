...
class GitHubRateLimitMonitor {
  constructor(token) {
    ...
    if (!this.token) {
      throw new Error('GitHub token not provided. Set GITHUB_TOKEN env var or use --token');
    }
    this.verbose = false;
  ...
  async getRestRateLimit() {
    try {
      const response = await this.request('GET', CONFIG.RATE_LIMIT_ENDPOINTS.REST);
      ...
      return {
        ...
        search: response.body.rate_limit.search || response.body.resources?.search,
        graphql: response.body.rate_limit.graphql || response.body.resources?.graphql,
        raw: response.body
      };
    } catch (error) {
  ...
  formatRateLimitData(data) {
    ...
    if (data.core) {
      ...
      formatted.core = {
        ...
        percent_remaining: ((data.core.remaining / data.core.limit) * 100).toFixed(2) + '%',
        reset_at: data.core.reset,
        reset_in: resetTime.formatted,
        nearly_exhausted: this.isNearlyExhausted(data.core.remaining, data.core.limit)
      };
    }
    ...
    if (data.search) {
      ...
      formatted.search = {
        ...
        percent_remaining: ((data.search.remaining / data.search.limit) * 100).toFixed(2) + '%',
        reset_at: data.search.reset,
        reset_in: resetTime.formatted,
        nearly_exhausted: this.isNearlyExhausted(data.search.remaining, data.search.limit)
      };
    }
    ...
    if (data.graphql) {
      ...
      formatted.graphql = {
        ...
        percent_remaining: ((data.graphql.remaining / data.graphql.limit) * 100).toFixed(2) + '%',
        reset_at: data.graphql.reset,
        reset_in: resetTime.formatted,
        nearly_exhausted: this.isNearlyExhausted(data.graphql.remaining, data.graphql.limit)
      };
    }

    if (data.rateLimit) {
      const resetTime = this.calculateTimeUntilReset(Math.floor(new Date(data.rateLimit.resetAt).getTime() / 1000));
      formatted.graphql = {
        ...
        reset_in: resetTime.formatted,
        nearly_exhausted: this.isNearlyExhausted(data.rateLimit.remaining, data.rateLimit.limit)
      };
    }
  ...
  print(message, level = 'info') {
    ...
    console.log(`${color}${prefix} [${level.toUpperCase()}]${colors.reset} ${message}`);
  }
  ...
  displayStatus(restData, graphqlData) {
    ...
    if (restData) {
      ...
      if (restData.core) {
        ...
        console.log(`     • Limit:      ${coreStatus.core.limit} requests/hour`);
        console.log(`     • Remaining:  ${coreStatus.core.remaining} (${coreStatus.core.percent_remaining})`);
      ...
      if (restData.search) {
        ...
        console.log(`\n   Search API:`);
        console.log(`     • Limit:      ${searchStatus.search.limit} requests/minute`);
        console.log(`     • Remaining:  ${searchStatus.search.remaining} (${searchStatus.search.percent_remaining})`);
        ...
        if (searchStatus.search.nearly_exhausted) {
          this.print(`   ⚠️  Search API rate limit nearly exhausted!`, 'warning');
        }
      }
    }
    ...
    if (graphqlData) {
      ...
      console.log(`   • Limit:      ${graphqlStatus.graphql.limit} points/hour`);
      console.log(`   • Remaining:  ${graphqlStatus.graphql.remaining} (${graphqlStatus.graphql.percent_remaining})`);
      ...
      if (graphqlStatus.graphql.nearly_exhausted) {
        this.print(`   ⚠️  GraphQL API rate limit nearly exhausted!`, 'warning');
      }
    }
  ...
  async checkRateLimits() {
    try {
      ...
      const restResult = restData.status === 'fulfilled' ? restData.value : null;
      const graphqlResult = graphqlData.status === 'fulfilled' ? graphqlData.value : null;

      if (!restResult && !graphqlResult) {
        this.print('Failed to check both REST and GraphQL rate limits', 'error');
        process.exit(1);
      }

      if (restData.status === 'rejected') {
        this.print(`REST API check failed: ${restData.reason.message}`, 'warning');
      }

      if (graphqlData.status === 'rejected') {
        this.print(`GraphQL API check failed: ${graphqlData.reason.message}`, 'warning');
      }
    ...
    try {
      ...
      while (true) {
        ...
        if (data.graphql) {
          ...
          if (graphqlFormatted.graphql?.nearly_exhausted) anyNearlyExhausted = true;
        }
  ...
  async monitor(intervalMinutes = 5) {
    this.print(`Starting rate limit monitoring (every ${intervalMinutes} minutes)...`, 'info');
  ...
  async reset() {
    try {
      this.print('Analyzing rate limit status for reset recommendations...', 'info');
      const data = await this.checkRateLimits();
      ...
      console.log('   • Authenticate with a Personal Access Token (already using token ✓)');
      ...
      console.log('   • Use GraphQL to reduce API calls (fetch multiple fields in one request)');
      console.log('   • Implement caching to avoid redundant requests');
      console.log('   • Use conditional requests (ETags) to avoid counting against limits');
      ...
      console.log('   • GitHub Apps: Can have higher limits per installation\n');

      console.log('5. IMPLEMENT RATE LIMIT AWARE RETRY LOGIC');
      console.log('   • Check rate_limit response headers before making requests');
      ...
      if (data.rest) {
        ...
        if (restFormatted.core) {
          ...
          console.log(`        (${new Date(restFormatted.core.reset_at * 1000).toLocaleString()})\n`);
        }
        if (restFormatted.search) {
          console.log(`  Search: Resets in ${restFormatted.search.reset_in}`);
          console.log(`          (${new Date(restFormatted.search.reset_at * 1000).toLocaleString()})\n`);
        }
      }
