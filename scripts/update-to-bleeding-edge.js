...
async function createBackup() {
  ...
  try {
    ...
    const backupFile = path.join(CONFIG.backupPath, `package-${timestamp}.json`);
...
async function updatePackages() {
  ...
  const packageJson = JSON.parse(await fs.readFile(CONFIG.packageJsonPath, 'utf8'));
  const updates = [];
  ...
  for (const depType of depTypes) {
    ...
    for (const pkg of packageNames) {
      ...
      if (currentVersion === latest.tag || currentVersion === latest.version) {
        log(`  ✅ ${pkg} is already on ${latest.tag} (${latest.version})`, 'green');
      } else {
...
async function applyUpdates(packageJson, updates) {
  ...
  for (const update of updates) {
    log(`  • ${update.package}: ${update.currentVersion} → ${update.newTag}`, 'blue');
  }
