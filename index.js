...
app.post(
  "/game/check",
  [
    body('gameId').isString().trim().notEmpty().withMessage('gameId must be a non-empty string'),
    body('guess').isInt({ min: 1, max: 100 }).withMessage('guess must be an integer between 1 and 100')
  ],
  (req, res) => {
    // Validate input
