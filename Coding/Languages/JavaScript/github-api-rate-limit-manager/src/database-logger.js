...
class DatabaseLogger {
  ...
  async initializeSqlite() {
    try {
      ...
      this.sqlite.run('CREATE INDEX IF NOT EXISTS idx_timestamp ON logs(timestamp)');
      this.sqlite.run('CREATE INDEX IF NOT EXISTS idx_tokenId ON logs(tokenId)');
    } catch (error) {
  ...
  saveJsonLogs() {
    try {
      fs.writeFileSync(this.logFile, JSON.stringify({ logs: this.logs }, null, 2));
    } catch (error) {
  ...
  async analyzeTrends(days = 7) {
    ...
    Object.entries(dailyStats).forEach(([date, stat]) => {
      ...
      const requests = stat.logs.reduce((sum, l) => sum + (l.requestCount || 0), 0);
      const errors = stat.logs.reduce((sum, l) => sum + (l.errorCount || 0), 0);

      trends[date] = {
        avgRemaining: Math.round(remaining.reduce((a, b) => a + b) / remaining.length),
  ...
  calculateTrendSummary(trends) {
    ...
    const avgRemaining = Math.round(values.reduce((sum, v) => sum + v.avgRemaining, 0) / values.length);
  ...
  async detectPeakUsage(days = 7, threshold = 100) {
    ...
    return {
      ...
      recommendation: peaks.length > 0 ? 'Consider adding more tokens during peak hours' : 'Usage is within normal limits'
    };
  }
  ...
  async exportToCSV(options = {}) {
    ...
    const csvPath = path.join(this.storagePath, `rate-limit-export-${Date.now()}.csv`);

    const headers = ['Timestamp', 'Token ID', 'Remaining', 'Limit', 'Percentage Used', 'Health', 'Requests', 'Errors'];
  ...
  async getMongoDBLogs(options) {
    ...
    return await collection.find(query).sort({ timestamp: -1 }).limit(options.limit).toArray();
  }
  ...
  async cleanupOldLogs(daysToKeep = 30) {
    ...
    if (this.type === 'json') {
      ...
      return new Promise((resolve) => {
        this.sqlite.run('DELETE FROM logs WHERE timestamp < ?', [cutoffDate], (err) => {
