...
class MultiTokenManager {
   ...
   */
  updateTeamStats () {
    this.teamStats.totalRequests = this.tokens.reduce(
      (sum, t) => sum + t.requestCount,
      0
    )
    this.teamStats.totalErrors = this.tokens.reduce(
      (sum, t) => sum + t.errorCount,
      0
    )
    this.teamStats.combinedRemaining = this.tokens.reduce(
      (sum, t) => sum + t.remaining,
      0
    )
    this.teamStats.combinedLimit = this.tokens.reduce(
      (sum, t) => sum + t.limit,
      0
    )
    this.teamStats.averageHealth =
  ...
  getTeamQuota () {
    return {
      ...
      healthStatus:
        this.teamStats.averageHealth > 70
          ? 'healthy'
          : this.teamStats.averageHealth > 40
            ? 'warning'
            : 'critical',
