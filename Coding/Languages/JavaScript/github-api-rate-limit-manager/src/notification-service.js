...
class NotificationService {
  ...
  async send (platform, payload) {
    const webhook =
      platform === 'slack' ? this.slackWebhook : this.discordWebhook
    ...
    switch (type) {
        ...
        fields.push(
          {
            ...
            value: `Token #${data.previousTokenId}`,
            short: true
          },
          {
            title: 'New Token',
            value: `Token #${data.newTokenId}`,
            short: true
          },
          {
            title: 'Previous Health',
            value: `${data.previousHealth}%`,
            short: true
          },
      ...
      case 'error':
        fields.push(
          ...
          {
            title: 'Status Code',
            value: data.statusCode?.toString() || 'N/A',
            short: true
          },
  ...
  async notifyWarning (data) {
    ...
    if (this.slackWebhook) {
      ...
      promises.push(
        this.sendWithRetry('slack', payload).catch((e) => ({
          error: e.message
        }))
      )
    }
    ...
    if (this.discordWebhook) {
      ...
      promises.push(
        this.sendWithRetry('discord', payload).catch((e) => ({
          error: e.message
        }))
      )
    }
