...
const resolvers = {
  Query: {
    metrics: async () => {
      try {
        codeQualityHistogram.observe({ file_type: 'javascript', severity: 'warning' }, 0.5);
    ...
    health: async () => {
      return JSON.stringify({ status: 'ok', service: 'code-quality-dashboard' });
    }
  }
};
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'code-quality-dashboard', timestamp: new Date().toISOString() });
});
