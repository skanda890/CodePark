  ...
  Mutation: {
    ...
    fixIssues: async (_, { ruleIds }) => {
      try {
        ...
        const fixedCount = results.reduce((sum, r) => sum + r.output ? 1 : 0, 0);
...
function processResults(results, limit = 100) {
  ...
  return {
    ...
    errorCount,
    warningCount,
    averageComplexity: files.reduce((sum, f) => sum + f.complexity, 0) / files.length || 0,
...
async function getIssues(severity, file) {
  ...
  results.forEach((result) => {
    result.messages?.forEach((msg, idx) => {
      const sev = msg.severity === 2 ? 'ERROR' : msg.severity === 1 ? 'WARNING' : 'INFO';
