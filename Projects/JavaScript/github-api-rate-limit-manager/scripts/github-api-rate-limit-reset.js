...
class GitHubRateLimitMonitor {
  constructor (token = process.env.GITHUB_TOKEN) {
    if (!token) {
      throw new Error(
        'GITHUB_TOKEN environment variable not set. Set it with: export GITHUB_TOKEN="your_token"'
      )
    }
    this.token = token
    this.config = config
  ...
  async displayLimits (jsonOutput = false) {
    ...
    console.log(
      `  ${coreHealth.emoji} Remaining: ${core.remaining}/${core.limit} requests`
    )
    ...
    console.log(
      `  Time remaining: ${this.formatTimeRemaining(core.reset * 1000)}`
    )
    ...
    console.log(this.colorize('\nSearch API:', 'bold'))
    console.log(
      `  ${searchHealth.emoji} Remaining: ${search.remaining}/${search.limit} requests`
    )
    console.log(`  Percentage: ${searchPercent}%`)
    console.log(
      `  Reset time: ${new Date(search.reset * 1000).toLocaleString()}`
    )
    console.log(
      `  Time remaining: ${this.formatTimeRemaining(search.reset * 1000)}`
    )
    ...
    console.log(this.colorize('\nGraphQL API:', 'bold'))
    console.log(
      `  ${gqlHealth.emoji} Remaining: ${gql.remaining}/${gql.limit} points`
    )
    console.log(`  Percentage: ${gqlPercent}%`)
    console.log(`  Reset time: ${new Date(gql.reset * 1000).toLocaleString()}`)
    console.log(
      `  Time remaining: ${this.formatTimeRemaining(gql.reset * 1000)}\n`
    )
    ...
    const checkOnce = async () => {
      try {
        ...
        if (core.remaining === 0) {
          ...
          console.log(
            `Time remaining: ${this.formatTimeRemaining(resetTime)}...`
          )
          return false
        } else {
          if (isRateLimited) {
            ...
            console.log(
              `Remaining: ${core.remaining}/${core.limit} requests\n`
            )
            return true
          }
  ...
  async getRecommendations () {
    ...
    if (corePercent < 20) {
      console.log(
        this.colorize('⚠ WARNING: Low REST API quota remaining!', 'red')
      )
    ...
    if (gqlPercent > 80) {
      console.log(
        this.colorize('✓ GraphQL API has good quota remaining', 'green')
      )
