...
class RequestQueue {
  ...
  enqueue (request, priority = 'normal') {
    ...
    const priorityLevel =
      this.PRIORITY[priority.toUpperCase()] || this.PRIORITY.NORMAL
    let added = false
    ...
    for (let i = 0; i < this.queues[priority].length; i++) {
      if (
        priorityLevel <
        this.PRIORITY[this.queues[priority][i].priority.toUpperCase()]
      ) {
  ...
  getQueueHealth () {
    ...
    const successRate =
      totalQueued > 0 ? (this.stats.totalSuccess / totalQueued) * 100 : 0
