...
export class InputValidator {
  ...
  validateExpression(expr, maxLength = this.config.maxExpressionLength) {
    ...
    if (expr.length > maxLength) {
      throw new Error(`Expression exceeds maximum length of ${maxLength} characters`);
    }
  ...
  validateArray(array, maxLength = this.config.maxArrayLength) {
    ...
    if (array.length > maxLength) throw new Error(`Array exceeds maximum length of ${maxLength}`);
  ...
  async load(key, decrypt = true) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = async () => {
        ...
        const value = decrypt ? await this.decryptData(request.result.data) : request.result.data;
        resolve(value);
      };
    });
  }
...
export class ErrorHandler {
  ...
  handle(error, context = 'Unknown') {
    ...
    return {
      error: this.isDevelopment ? error.message : 'An unexpected error occurred',
