...
class TeamService {
  ...
  async addTeamMember(projectId, userId, role = 'CONTRIBUTOR') {
    try {
      ...
      // Log action
      await this.logAuditEvent(projectId, 'team_member_added', 'member', member.id);
  ...
  async updateMemberRole(projectId, userId, newRole) {
    ...
    await this.logAuditEvent(projectId, 'member_role_updated', 'member', member.id, 
      member.role, newRole);
  ...
  async hasPermission(projectId, userId, requiredPermission) {
    ...
    const rolePermissions = {
      'OWNER': ['all'],
      'ADMIN': ['manage-team', 'manage-settings', 'delete-project', 'manage-reviews'],
      'MAINTAINER': ['create-review', 'approve-review', 'merge-code', 'manage-issues'],
   ...
   * Log audit event
   */
  async logAuditEvent(projectId, action, resourceType, resourceId, oldValue = null, newValue = null) {
