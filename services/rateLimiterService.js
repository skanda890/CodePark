...
class RateLimiter {
  ...
  async checkLimit(key, options = {}) {
    ...
    try {
      return await this._checkSlidingWindow(slidingWindowKey, windowMs, maxRequests);
    } catch (error) {
      logger.error('Rate limiter check failed', { key, error: error.message });
      // Fail open - allow request if Redis is down
      return { allowed: true, warning: 'Rate limiter unavailable', retryAfter: null };
    }
  }
  ...
  async checkTokenBucket(key, options = {}) {
    const { ok, tokenBucketKey, error } = this._validateKey(key, 'checkTokenBucket');
    if (!ok) return { allowed: false, error, retryAfter: null };
    ...
    try {
      return await this._checkTokenBucket(tokenBucketKey, capacity, refillRate, tokensPerRequest);
    } catch (error) {
      logger.error('Token bucket check failed', { key, error: error.message });
      return { allowed: true, warning: 'Token bucket unavailable', retryAfter: null };
    }
  }
